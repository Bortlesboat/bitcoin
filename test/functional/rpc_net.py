...
class NetTest(BitcoinTestFramework):
    ...
    def test_addnode_connection_limit(self):
        ...
        # Must match MAX_ADDNODE_CONNECTIONS in src/net.h
        MAX_ADDNODE = 8
        node = self.nodes[0]
        ...
        targets = MAX_ADDNODE + 1  # 9 targets (nodes 2-10), only 8 can connect

        self.log.info("Verify clean initial state")
        assert_equal(node.getaddednodeinfo(), [])
        assert_equal(self.count_manual(node), 0)

        self.log.info(f"Add {targets} nodes to the addnode list")
        ...
        self.log.info(f"Wait for {MAX_ADDNODE} manual connections to establish")
        self.wait_until(lambda: self.count_manual(node) >= MAX_ADDNODE, timeout=120)
        assert_equal(self.count_manual(node), MAX_ADDNODE)

        self.log.info("Verify exactly one target node was NOT connected")
        inbound_counts = [len(self.nodes[i].getpeerinfo()) for i in range(2, 2 + targets)]
        assert_equal(inbound_counts.count(0), 1)
        assert_equal(inbound_counts.count(1), MAX_ADDNODE)
        ...
        self.wait_until(lambda: self.count_manual(node) >= MAX_ADDNODE, timeout=120)
        assert_equal(self.count_manual(node), MAX_ADDNODE)
        assert_equal(len(node.getaddednodeinfo()), MAX_ADDNODE)
